...
contract DOGEBROS is Ownable, IBEP20 {
    ...
    string constant _name = "Super Doge Bros";
    string constant _symbol = "DOGEBROS";
    ...
    uint256 devFee = 0;
    uint256 public totalFee = liquidityFee.add(devFee).add(burnFee);
    ...
    function swapAndLiquify(uint256 amount) private swapping {
        ...
        uint256 amountLiqBNB = (newBalance.mul(liquidityFee.div(2))).div(tfee);
    ...
    function setFees(uint256 _liquidityFee, uint256 _devFee, uint256 _burnFee) external onlyOwner {
        liquidityFee = _liquidityFee;
        devFee = _devFee;
        burnFee = _burnFee;
        totalFee = _liquidityFee.add(_devFee).add(burnFee);
    ...
    function setBuyback(bool _enabled, uint256 _buyBackUpperLimit) external onlyOwner {
